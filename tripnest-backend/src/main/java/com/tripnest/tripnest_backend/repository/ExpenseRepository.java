...
public interface ExpenseRepository
    ...
    // Total expenses across multiple trips
    @Query("""
        SELECT COALESCE(SUM(e.amount), 0)
        FROM Expense e
        WHERE e.tripId IN :tripIds
    """)
    BigDecimal getTotalExpensesByTripIds(
            @Param("tripIds") List<Long> tripIds
    );

    // Category summary across multiple trips
    @Query("""
        SELECT e.category, SUM(e.amount)
        FROM Expense e
        WHERE e.tripId IN :tripIds
        GROUP BY e.category
    """)
    List<Object[]> getCategorySummaryByTripIds(
            @Param("tripIds") List<Long> tripIds
    );

    // Total expenses amount across entire platform
    @Query("""
        SELECT COALESCE(SUM(e.amount), 0)
        FROM Expense e
    """)
    BigDecimal getTotalPlatformExpenseAmount();
}