...
function TripDetails() {
  ...
  const handleAddMemberSubmit = async (e) => {
    ...
    try {
      ...
      const message =
        ...
        "Unable to add member. Please verify the email and try again.";
  ...
  const fetchBudget = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/api/trips/${id}/budget`,
        getAuthConfig()
      );
  ...
  const fetchExpenses = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/api/trips/${id}/expenses`,
        getAuthConfig()
      );
  ...
  const fetchRemainingBudget = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8080/api/trips/${id}/budget/remaining`,
        getAuthConfig()
      );
  ...
  const fetchCategorySummary = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8080/api/trips/${id}/expenses/summary/category`,
        getAuthConfig()
      );
  ...
  const handleSaveExpense = async (e) => {
    ...
    try {
      ...
      if (editingExpense) {
        await axios.put(
          `http://localhost:8080/api/trips/${id}/expenses/${editingExpense.id}`,
          payload,
          getAuthConfig()
        );
      } else {
  ...
  const handleDeleteExpense = async (expenseId) => {
    ...
    try {
      ...
      await axios.delete(
        `http://localhost:8080/api/trips/${id}/expenses/${expenseId}`,
        getAuthConfig()
      );
      await fetchExpenses();
      await fetchRemainingBudget();
        ...
        >
          <div style={styles.modalContainer} className="modal-content-animate">
            <div style={styles.modalHeader}>
              <div>
                <h3 style={styles.modalHeading}>👥 Add Trip Member</h3>
                <p style={styles.modalSubheading}>Invite a traveler by registered email</p>
              </div>
              <button
